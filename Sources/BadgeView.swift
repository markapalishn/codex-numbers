...
final class BadgeView: ClickableBadge {
    var usage: Usage? {
        didSet {
            needsDisplay = true
            setAccessibilityLabel((usage?.line ?? "Ожидание Codex") + ". Открыть аналитику")
        }
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        if let remaining, remaining > 0 {
            ...
            arc.appendArc(withCenter: NSPoint(x: ringX+8, y: 30), radius: 8, startAngle: -90, endAngle: -90 + 360 * CGFloat(remaining)/100, clockwise: false)
            limitColor.setStroke(); arc.stroke()
        }
        text("Остаток лимита", x: ringX + 28, y: 10, font: small, color: .secondaryLabelColor)
        text(remaining.map { "\($0)%" } ?? "—", x: ringX + 27, y: 25, font: Self.numberFont, color: .labelColor)
